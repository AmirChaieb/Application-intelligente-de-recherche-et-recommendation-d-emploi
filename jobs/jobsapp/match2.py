...
def configure_gemini():
    ...
    if not config2.GEMINI_API_KEY:
        raise GeminiError("Gemini API Key is not configured in .env file.")
    try:
        genai.configure(api_key=config2.GEMINI_API_KEY)
...
def analyze_resume_jd(resume_text, jd_text):
    ...
    prompt = f"""
You are an HR AI assistant. Analyze the resume and job description below and determine their compatibility.
...
Respond STRICTLY in the following format (do NOT change titles):
...
PERCENTAGE_MATCH:
[number only between 0 and 100, no % symbol, no explanation]
IMPORTANT: Do not skip any section.
...
"""
    ...
    try:
        ...
        percentage_match = re.search(r"PERCENTAGE_MATCH.*?(\d{1,3})\s*%?", text, re.IGNORECASE | re.DOTALL)
